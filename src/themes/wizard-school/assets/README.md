...
## Third-party sound effects — LICENCE NOT VERIFIED

`sounds/` holds six clips MM supplied. Filenames preserve what appear to be the
original contributor handles, which is the only provenance information the files
carry:

| File | Used for | Apparent source |
|---|---|---|
| `matthewvakaliuk73627-mouse-click.mp3` | every button click | contributor handle |
| `daviddumais-magical-spell-cast.mp3` | START | contributor handle |
| `freesound-clockwork-timer.mp3` | PAUSE | Freesound |
| `biww-fire-burst-flame-sound-effect.mp3` | New candle | contributor handle |
| `cat.mp3` | choosing the cat | unknown |
| `lazychillzone-owl-hooting.mp3` | choosing the owl | contributor handle |

**Nobody has confirmed the licence terms for any of these, and this repo is
public and deployed.** Before launch, confirm each clip's licence and whether it
requires attribution, and record the answer here. I can report what the files are;
I can't tell you whether using them is lawful.

`spell-complete.wav` is exempt — it is generated by this repo (see below) and has
no third party attached.
